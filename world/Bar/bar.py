# -*- coding: utf-8 -*-
"""
Created on Sun Sep 15 16:42:09 2019
...
list_data=[]
# 指定要列出所有檔案的目錄
mypath = "D:/Apache/Apache24/htdocs/world/News/db/"
...
# 以迴圈處理
while monthtime < ticks:
    monthtime=monthtime+86400
    T=time.strftime("%Y.%m.%d",time.localtime(monthtime) )
    for name in files:
  # 產生檔案的絕對路徑
      fullpath = join(mypath, name)
  # 判斷 fullpath 是檔案還是目錄
      if isfile(fullpath):
...
#[0, "爆炸"], [1, "水災"], [2, "乾旱"], [3, "恐攻"], [4, "戰爭"], [5,"抗議" ]
  
for country in list_Country.keys():
    ...
    for data in list_data:
        ...
        file_1 = open('../News/db/'+list_data[0], "r", encoding="utf-8-sig")
        data_1 = file_1.read()
        jsondata_1 = json.loads(data_1)
        file_1.close()
        for content in jsondata_1:
            if (content["Country_chin"]==country):
                if content["Event"]!="目前無相關資訊":
                    
                    for event in content["Event"]:
                    
                        if event["Title"] not in list_title:
                            list_title.append(event["Title"])
                            M=event["Title"]
    ...
    list_B[country]=[[0, a], [1, b], [2, c], [3, d], [4, e], [5,f ]]         


with codecs.open('callBackdata2.json','w',"utf-8") as f:#給網頁讀取資訊的檔案
      f.write('callBackdata2(JSON.stringify(')
      f.write(json.dumps(list_B,ensure_ascii=False))
      f.write('));')    